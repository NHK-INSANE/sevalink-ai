...
function ChatContent() {
  ...
  const fetchData = async (currentUser) => {
    ...
    try {
      ...
      if (targetUserId) {
        const existing = chatRes.data.find(c => c.type === "direct" && c.participants.some(p => p._id === targetUserId));
        if (existing) {
  ...
  useEffect(() => {
    if (user && selectedChat) {
      fetchMessages();
      socket.emit("join_problem", selectedChat._id);
    }
  }, [selectedChat, user]);
  ...
  const filteredItems = activeTab === "direct" ? chats : missions;
          ...
          <div className="w-full md:w-[350px] flex flex-col border-r border-white/5 bg-[#0f172a]/20">
            <div className="p-6 border-b border-white/5 space-y-4">
              ...
              <div className="flex bg-black/40 p-1 rounded-xl">
                 <button onClick={() => setActiveTab("direct")} className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === "direct" ? "bg-indigo-600 shadow-md text-white" : "text-gray-500 hover:text-gray-300"}`}>Direct Ops</button>
                 <button onClick={() => setActiveTab("team")} className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${activeTab === "team" ? "bg-purple-600 shadow-md text-white" : "text-gray-500 hover:text-gray-300"}`}>Mission Units</button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              {loading ? (
                ...
                <div className="p-10 text-center text-[10px] font-black uppercase tracking-widest text-gray-600 border border-dashed border-white/5 m-6 rounded-2xl">
                  No Active {activeTab === "direct" ? "Direct" : "Mission"} Channels
                </div>
              ) : (
                filteredItems.map(c => {
                  ...
                  return (
                    <div key={c._id} onClick={() => setSelectedChat(c)} className={`p-5 cursor-pointer transition-all flex items-center gap-4 border-b border-white/5 relative ${isSelected ? "bg-white/5" : "hover:bg-white/[0.02]"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <span className={`font-bold text-sm truncate ${isSelected ? "text-white" : "text-gray-300"}`}>{other.name}</span>
                        ...
                        <div className="text-[11px] text-gray-500 truncate font-medium">{c.lastMessage?.text || c.lastMessage?.message || "Secure Channel Open..."}</div>
                      </div>
                    </div>
                  );
                })
            ...
            {selectedChat ? (
              <>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-black text-white tracking-tight flex items-center gap-2">
                        ...
                        {selectedChat.type === "team" && <span className="text-[8px] bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded border border-purple-500/20 uppercase">Tactical Group</span>}
                      </h3>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar relative">
                  {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      ...
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest max-w-[250px] mx-auto leading-relaxed">Neural protocols active. Data retained for 14 days.</p>
                    </div>
                  ) : (
                    messages.map((m, idx) => {
                      ...
                      return (
                        <div key={m._id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`max-w-[75%] px-5 py-3 rounded-2xl shadow-xl ${isMe ? "bg-indigo-600 text-white rounded-tr-sm" : "bg-[#1e293b] text-gray-200 rounded-tl-sm border border-white/5"}`}>
                            {!isMe && selectedChat.type === "team" && <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1.5">{m.senderName || "Personnel"}</p>}
                ...
                <div className="p-6 bg-[#0f172a]/90 backdrop-blur-xl border-t border-white/5 flex items-center gap-4 z-10">
                  <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} placeholder="Type secure message..." className="flex-1 bg-black/40 border border-white/10 rounded-[1.5rem] px-6 py-4 text-sm text-white placeholder-gray-600 outline-none focus:border-indigo-500/50 transition-all shadow-inner" />
                  <button onClick={() => sendMessage()} disabled={!inputText.trim()} className="w-14 h-14 rounded-[1.5rem] bg-indigo-600 text-white flex items-center justify-center hover:bg-indigo-500 disabled:opacity-20 transition-all active:scale-90 shadow-lg shadow-indigo-500/20">➤</button>
                </div>
              </>
            ) : (
