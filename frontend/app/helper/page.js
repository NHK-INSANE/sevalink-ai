...
export default function Helper() {
  ...
  useEffect(() => {
    // For now dummy data (no backend change)
    setHelpers([
      { name: "Rahul", role: "Volunteer", skill: "Food Supply", icon: "🤝" },
      { name: "Amit NGO", role: "NGO Worker", skill: "Medical", icon: "🏢" }
    ]);
  }, []);
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-slate-400">
            Dedicated individuals and NGO workers ready to assist in crisis situations.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {helpers.map((h, i) => (
            <div key={i} className="glass p-6 rounded-2xl border border-white/10 hover:border-indigo-500/30 transition-all group">
        ...
        {helpers.length === 0 && (
          <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10">
            <p className="text-slate-500">No helpers found nearby.</p>
          </div>
        )}
      </main>
    </div>
